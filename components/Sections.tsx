...
const Button: React.FC<{ 
  ...
  href?: string;
  type?: "button" | "submit";
}> = ({ children, variant = 'primary', className = '', onClick, asAnchor, href, type = "button" }) => {
  ...
  const styles = {
    primary: `${COLORS.primary} text-white hover:opacity-90 shadow-lg shadow-[#8A9A8A]/20`,
    secondary: `${COLORS.secondary} text-[#2D3436] hover:opacity-90`,
    outline: "border-2 border-[#8A9A8A] text-[#8A9A8A] hover:bg-[#8A9A8A] hover:text-white",
  ...
  return (
    <button type={type} onClick={variant !== 'disabled' ? onClick : undefined} className={fullClassName}>
      {children}
    </button>
  );
};
...
const SectionHeading: React.FC<{ title: string; subtitle?: string; center?: boolean }> = ({ title, subtitle, center }) => (
  <div className={`mb-6 ${center ? 'text-center' : 'text-left'}`}>
    ...
    <div className={`h-1 w-12 bg-[#8A9A8A] mt-3 ${center ? 'mx-auto' : ''}`} />
  </div>
);
        ...
        <div className="flex justify-between h-16 items-center py-2">
          <div className="flex items-center">
            <a href="/" className="group flex items-center gap-2.5 transition-all duration-300">
              <div className="w-8 h-8 rounded-xl bg-[#8A9A8A] flex items-center justify-center shadow-sm group-hover:shadow-[#8A9A8A]/30 group-hover:-rotate-6 transition-all">
          ...
          <div className="hidden md:flex space-x-8 items-center">
            {NAVIGATION.map((item) => (
              <a 
                key={item.name} 
                href={item.href} 
                className="text-gray-400 hover:text-[#8A9A8A] transition-colors font-semibold text-[10px] uppercase tracking-[0.2em]"
              >
                {item.name}
              </a>
            ))}
        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-6 space-y-4 shadow-2xl animate-in fade-in slide-in-from-top-4">
          {NAVIGATION.map((item) => (
            <a 
              ...
              className="block text-lg text-gray-700 hover:text-[#8A9A8A] font-medium"
            >
              {item.name}
            </a>
          ))}
...
export const Hero: React.FC = () => (
  <section className="relative pt-24 pb-12 md:pt-32 md:pb-24 bg-[#FAFAF8] overflow-hidden">
    <div className="absolute top-0 right-0 w-full h-full pointer-events-none opacity-40">
      <div className="absolute top-1/2 right-[-5%] w-[350px] h-[350px] md:w-[500px] md:h-[500px] bg-[#8A9A8A]/20 rounded-full blur-[80px] animate-pulse" />
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      <div className="flex flex-col md:flex-row items-center gap-8 lg:gap-16">
        <div className="md:w-3/5 text-center md:text-left">
          <div className="inline-flex items-center gap-2.5 px-3 py-1 rounded-full bg-[#8A9A8A]/15 text-[#8A9A8A] text-[10px] font-bold uppercase tracking-[0.15em] mb-6 animate-in fade-in slide-in-from-bottom-2">
        ...
        <div className="md:w-2/5 flex justify-center items-center relative py-6">
          <div className="relative w-56 h-56 md:w-72 md:h-72">
            <div className="absolute inset-0 border-2 border-[#8A9A8A]/30 rounded-full animate-[spin_20s_linear_infinite]" />
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 bg-white rounded-full shadow-2xl flex items-center justify-center z-20">
               <div className="w-28 h-28 rounded-full bg-[#8A9A8A]/10 flex items-center justify-center animate-pulse">
                  <Sparkles className="w-8 h-8 text-[#8A9A8A]" />
               </div>
            </div>
            ...
            <div className="absolute bottom-10 -left-12 bg-white px-3 py-1.5 rounded-xl shadow-lg border border-gray-50 flex items-center gap-1.5 z-30 animate-bounce delay-700">
              <Check className="w-2.5 h-2.5 text-[#8A9A8A]" />
      ...
      <div className="flex flex-col md:flex-row gap-12 items-center">
        <div className="md:w-5/12">
          <div className="relative group">
            ...
            />
            <div className="absolute -bottom-6 -right-6 bg-[#8A9A8A] text-white p-5 rounded-2xl shadow-xl hidden md:block">
        ...
        <div className="md:w-7/12">
          <div className="mb-6">
            <h4 className="text-[#8A9A8A] text-[10px] font-bold uppercase tracking-[0.3em] mb-2">Poznajmy się</h4>
            <h2 className="text-2xl md:text-3xl font-semibold text-[#2D3436] mb-4">{ABOUT_ME.name}</h2>
            <p className="text-xs font-medium text-[#8A9A8A] mb-4 uppercase tracking-widest">{ABOUT_ME.role}</p>
          </div>
          ...
          <div className="space-y-3">
             ...
             <div className="grid grid-cols-1 gap-2">
                {ABOUT_ME.qualifications.slice(0, 6).map((q, i) => (
                  <div key={i} className="flex gap-3 items-start p-3 bg-[#FAFAF8] rounded-xl border border-gray-50 hover:border-[#8A9A8A]/30 transition-all">
                    <Check className="w-3.5 h-3.5 text-[#8A9A8A] mt-0.5 shrink-0" />
  ...
  <section id="specjalizacja" className="py-16 bg-[#FAFAF8]">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="mb-12 mt-8">
        <h4 className="text-center font-bold text-[#8A9A8A] text-[9px] uppercase tracking-[0.4em] mb-6">Główna Specjalizacja</h4>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {SPECIALIZATIONS.main.map((item, i) => (
            <div key={i} className="bg-white p-6 rounded-2xl text-center border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-500 group">
              <div className="w-10 h-10 bg-[#8A9A8A]/5 rounded-xl flex items-center justify-center mb-4 mx-auto group-hover:bg-[#8A9A8A] transition-colors">
                <Check className="text-[#8A9A8A] group-hover:text-white w-4 h-4" />
              </div>
      ...
      <div className="max-w-4xl mx-auto">
        <h4 className="text-center font-bold text-[#8A9A8A] text-[9px] uppercase tracking-[0.4em] mb-6">Możliwe obszary wsparcia</h4>
        <div className="flex flex-wrap justify-center gap-2">
          {SPECIALIZATIONS.others.map((item, i) => (
            <span key={i} className="bg-white px-4 py-2 rounded-full border border-gray-100 text-[11px] text-gray-500 font-medium shadow-sm hover:border-[#8A9A8A] hover:text-[#8A9A8A] transition-all cursor-default">
              {item}
            </span>
          ))}
        </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col md:flex-row items-center gap-12">
          ...
          <div className="space-y-4 mt-8">
            {[
              ...
              <div key={i} className="flex gap-4 p-4 bg-[#FAFAF8] rounded-2xl border border-gray-50 hover:bg-white hover:shadow-lg transition-all duration-300">
                <div className="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center text-[#8A9A8A] shrink-0">
                  {step.icon}
                </div>
                <div>
        ...
        <div className="md:w-1/2 bg-[#2D3436] p-10 md:p-12 rounded-[3rem] text-white shadow-3xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-32 h-32 bg-[#8A9A8A]/10 rounded-full blur-2xl -mt-16 -mr-16 transition-all group-hover:bg-[#8A9A8A]/30 duration-700" />
          ...
          <ul className="space-y-4">
            {['Prywatność i 100% poufność', 'Komfort własnego domu', 'Zgodność z RODO i etyką zawodu'].map((item, i) => (
              <li key={i} className="flex items-center gap-3 font-medium text-xs">
                <div className="w-5 h-5 bg-[#8A9A8A] rounded-full flex items-center justify-center shrink-0">
...
export const Offer: React.FC = () => (
  <section id="oferta" className="py-16 bg-[#FAFAF8]">
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      <SectionHeading title="Cennik" center subtitle="Każda sesja to inwestycja w Twoje lepsze samopoczucie." />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
        {PRICES.map((item, i) => (
          <div key={i} className={`p-8 rounded-[2.5rem] border transition-all duration-700 flex flex-col ${item.isAvailable ? 'bg-white border-[#8A9A8A]/50 shadow-xl hover:-translate-y-2' : 'bg-gray-100 border-gray-200 opacity-60'}`}>
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-lg font-bold text-[#2D3436]">{item.title}</h3>
              {item.isAvailable ? (
                <span className="bg-[#8A9A8A]/10 text-[#8A9A8A] text-[9px] uppercase tracking-widest px-2.5 py-1 rounded-full font-bold">Dostępna</span>
              ) : (
                <span className="bg-gray-200 text-gray-400 text-[9px] uppercase tracking-widest px-2.5 py-1 rounded-full font-bold">Wkrótce</span>
              )}
            </div>
            ...
            <ul className="space-y-3 mb-8 flex-grow">
              {item.features.map((feat, j) => (
                <li key={j} className="flex items-center gap-2.5 text-xs text-gray-600">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#8A9A8A]" /> {feat}
                </li>
              ))}
            </ul>
            <Button asAnchor={item.isAvailable} href={item.isAvailable ? "#kontakt" : undefined} variant={item.isAvailable ? 'primary' : 'disabled'} className="w-full py-3.5 text-[10px] font-bold uppercase tracking-widest">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-10">
          {FEATURES.map((item, i) => (
            <div key={i} className="flex flex-col items-center group">
              <div className="w-14 h-14 bg-white rounded-xl shadow-sm flex items-center justify-center mb-4 text-[#8A9A8A] group-hover:scale-110 group-hover:bg-[#8A9A8A] group-hover:text-white transition-all duration-500">
                {item.icon}
              </div>
        ...
        {TESTIMONIALS.map((item, i) => (
          <div key={i} className="bg-white p-8 rounded-[2rem] italic border border-gray-100 shadow-lg relative group">
            ...
            <div className="flex items-center gap-3 relative z-10 pl-4">
              <div className="w-8 h-8 rounded-full bg-[#8A9A8A]/10 flex items-center justify-center font-bold text-[#8A9A8A] text-[10px]">
                {item.author.charAt(0)}
              </div>
              <span className="text-[#8A9A8A] font-bold not-italic text-[10px] uppercase tracking-widest">{item.author}</span>
            </div>
          </div>
        ))}
      </div>
...
export const FirstVisit: React.FC = () => (
  <section className="py-16 bg-white">
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
      <div className="bg-[#8A9A8A] p-10 md:p-16 rounded-[3rem] text-white shadow-3xl relative overflow-hidden group">
        ...
        <Button asAnchor href="#kontakt" variant="primary" className="!bg-white !text-[#8A9A8A] hover:!bg-[#2D3436] hover:!text-white !px-10 !py-4 shadow-2xl uppercase tracking-widest text-[10px] font-bold">
        ...
        <div className="space-y-3 mt-10">
          {FAQS.map((faq, i) => (
            <div key={i} className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm transition-all duration-300">
              ...
              >
                <span className="font-bold text-xs md:text-sm text-[#2D3436] group-hover:text-[#8A9A8A] transition-colors">{faq.question}</span>
                <div className={`w-7 h-7 rounded-full border border-gray-100 flex items-center justify-center transition-all ${openIndex === i ? 'bg-[#8A9A8A] border-[#8A9A8A] rotate-45' : ''}`}>
...
export const Contact: React.FC = () => {
  ...
  const handleSend = (e: React.FormEvent) => {
    e.preventDefault();
    const subject = encodeURIComponent(`Wiadomość ze strony Harmonia od: ${formData.name}`);
    const body = encodeURIComponent(`Imię: ${formData.name}\nEmail: ${formData.email}\n\nWiadomość:\n${formData.message}`);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-12">
          <div className="lg:w-1/2">
            <SectionHeading title="Umów wizytę" subtitle="Zapraszam do kontaktu w celu ustalenia terminu pierwszej konsultacji. Możesz napisać bezpośrednio lub skorzystać z formularza." />
            <div className="space-y-6 mt-10">
              <div className="flex gap-4 items-center p-5 rounded-2xl border border-gray-50 hover:bg-[#FAFAF8] transition-colors duration-500">
                <div className="w-12 h-12 bg-[#EDE7DE]/50 rounded-xl flex items-center justify-center text-[#8A9A8A] shrink-0">
                  <Phone className="w-5 h-5" />
                </div>
                <div>
                  <h4 className="font-bold text-[10px] text-[#2D3436] uppercase tracking-widest mb-0.5">Zadzwoń do mnie</h4>
              ...
              <div className="flex gap-4 items-center p-5 rounded-2xl border border-gray-50 hover:bg-[#FAFAF8] transition-colors duration-500">
                ...
                <div>
                  <h4 className="font-bold text-[10px] text-[#2D3436] uppercase tracking-widest mb-0.5">Napisz e-mail</h4>
          ...
          <div className="lg:w-1/2">
            <form className="bg-[#FAFAF8] p-8 md:p-12 rounded-[3rem] space-y-4 border border-gray-100 shadow-2xl" onSubmit={handleSend}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    className="w-full bg-white border border-gray-100 rounded-2xl px-5 py-3 text-xs focus:outline-none focus:ring-2 focus:ring-[#8A9A8A]/40 transition-all" 
                  />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <label className="block text-[9px] font-bold text-gray-400 uppercase tracking-[0.2em] ml-2">Twoja wiadomość</label>
                <textarea 
                  required
                  rows={3} 
                  placeholder="Napisz krótko, jakiego wsparcia potrzebujesz..." 
                  value={formData.message}
                  onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                  className="w-full bg-white border border-gray-100 rounded-2xl px-5 py-3 text-xs focus:outline-none focus:ring-2 focus:ring-[#8A9A8A]/40 transition-all resize-none"
                ></textarea>
              </div>
              <Button type="submit" variant="primary" className="w-full py-4 uppercase tracking-[0.2em] text-[10px] font-bold mt-2">Wyślij wiadomość</Button>
              <p className="text-[9px] text-gray-400 text-center leading-relaxed mt-2">
                Po kliknięciu otworzy się Twój program pocztowy. Dane są chronione zgodnie z RODO.
              </p>
            </form>
          </div>
        </div>
